(function IslandClass(ng, $, $w, PIXI, Swiper) {
    ...
    function EnvirontmentCtrl($scope, $rootScope, $ionicLoading, $ionicPopup, AppService) {
        ...
        $scope.closeMenu = function () {
            ...
            //-- hapus ticker timer action status
            if (window.GameApp.IslandBusiness.tickerEvent.length > 0) {
                //console.log(window.GameApp.IslandBusiness.tickerEvent);
                for (var i = 0; i < window.GameApp.IslandBusiness.tickerEvent.length ; i++) {
                    window.removeEventListener('serverdatetick', window.GameApp.IslandBusiness.tickerEvent[i]);
                    delete window.GameApp.IslandBusiness.tickerEvent[i];
                }
                window.GameApp.IslandBusiness.tickerEvent = [];
            }
        };
        ...
        $scope.killChar = function (playerCharId) {
            ...
            $ionicPopup.confirm({ title: 'Confirmation', template: 'Really want to kill that char ?' })
              .then(function (res) {
                  if (res) {
