(function IslandBusiness(win) {
    ...
    //-- di inject di game-island-environtment.js
    ...
    */
    _this.fetchRegionData = function (regionId, callback) {
        ...
        $http.post(url, { regionId: regionId }).then(function onSuccess(response) {
            ...
            //-- on failed
            console.log('regiondata error :', response);
        ...
        $http.post(url,
          {
              ...
              //-- on failed
              _this.fetchRegionData(scopeEnvi.island.id, callback);
              console.log('failed setupFarming');
          }).finally(function onFinally() {
              //$ionicLoading.hide();
            ...
            switch (targetNameValue.farmingStatus) {
                case 1:
                    ...
                    $.each(charsInEnvi, function (index, value) {
                        ...
                        if (isCharCanExploring && isLifespanEnough) {
                ...
                case 2:
                    ...
                    var thisTicker = function () {
                        ...
                        console.log('ticker ' + scopeEnvi.island.id + ' ' + targetName);

                        if (countdownJob.total <= 0) {
                            window.removeEventListener('serverdatetick', thisTicker);
                            _this.fetchStatusInfo();
                            scopeEnvi.closeMenu();
                        }
                    };
                    $w.addEventListener('serverdatetick', thisTicker);
                    $w.GameApp.IslandBusiness.tickerEvent.push(thisTicker);
                    ...
                    break;
                case 3:
                    obj[targetName].infoText = 'Keep hatching to get monster with ' + destroyingName + ' ability.';
                    //-- cek apakah char punya si player ada yg target wood  dan explorer gak
                    $.each(charsInEnvi, function (index, value) {
                        var isCharCanDestroying = checkAvailabilityChar(value, targetName, 'destroyer');
                        //-- inject validasi nyawa char
                        var isLifespanEnough = $w.GameApp.IslandEnviAnim.validateIsLifespanCharCanWorking(value.id);
                        if (isCharCanDestroying && isLifespanEnough) {
                        ...
                        //console.log(isLifespanEnough, value.nickname);
                    });
                    break;
                case 4:
                    //-- hitung countdown char kerja dan remaining diamond
        ...
        $http.post(url,
          {
              ...
              //-- on failed
              _this.fetchRegionData(scopeEnvi.island.id, function () { });
              console.log('failed finishNowFarming');
          }).finally(function onFinally() {
              //$ionicLoading.hide();
        ...
        $http.post(url,
          {
              ...
              //-- on failed
              _this.fetchRegionData(scopeEnvi.island.id, function () { });
              console.log('failed collectFarmingReward');
        ...
        $http.post(url,
          {
              ...
              //-- on failed
              window.GameApp.rootScope.reloadPlayerData();
              console.log('failed purchasingDecoration');
