...
(function ($w) {
	...
	var gameTimer = (function () {
		...
		timer1.start = function () {
			var startTimer = function () {
				timer1.interval = $w.cordova.plugins.TimerPlugin.addInterval(timer1.callback, 1000, function (id) {
					timer1.id = id;
				}, startTimer);
			};
			startTimer();
		...
		timer2.caller = {};
		timer2.isStop = false;
		timer2.interval = null;
		timer2.callback = function () { };
		timer2.start = function () {
			var settings = {
				timerInterval: 1000, // interval between ticks of the timer in milliseconds (Default: 10000)
				...
				notificationTitle: '', // title for the notification (Default: '')
				notificationText: '' // text for the notification (Default: '')
			};
			var startTimer = function () {
				window.BackgroundTimer.start(timer2.callback, startTimer, settings);
			};
		...
		timer2.stop = function () {
			var stopTimer = function () {
				window.BackgroundTimer.stop(function () { }, stopTimer);
			};
			stopTimer();
		};
		timer2.on = function (evt, callback) {
	...
	mg.wave = (function () {
		...
		_this.timeBusiness = (function () {
			...
			tb.startTime = function () {
				//$w.removeEventListener('serverdatetick', tb.ticker);
				//tb.countdown = speedGame.duration;
	...
	mg.createWorld = function () {
		...
		var mainScene = (function () {
			...
			containerGame.addChild(pauseBtn);
	...
	$w.addEventListener('minigameready', function () {
		...
		//globalEvent.showCoutdownTransition(function () { console.log('ready to play'); });
	});
