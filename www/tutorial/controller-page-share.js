...
(function ShareClass(w) {
    ...
    function ShareCtrl($scope, $rootScope, $http, $ionicLoading) {
        var renderer;
        var renderer2;
        ...
        $scope.showEggCode = function (charCode) {
            ...
            window.GameApp.SpreadCode.showQRCode(charCode, function () {
                ...
                window.GameApp.SpreadCode.renderer.view.addEventListener('click', $scope.showCharList);
        ...
        $scope.__showReward = function (charCode) {
            if (typeof (charCode) == 'undefined') {
                return;
            }
            var url = $w.GameApp.serverURL + '/reward/fetchdata';

            $ionicLoading.show();
            $http.post(url, { eggcode: charCode }).then(function onSuccess(response) {
                var rewardDatae = response.data.Reward;
                $scope.imagecharachter = rewardDatae.imagecharachter;
                $scope.nickname = rewardDatae.nickname;
                $scope.codechar = rewardDatae.codechar;
                $scope.numberofhatch = rewardDatae.numberofhatch;
                $scope.coinearned = rewardDatae.coinearned;
                $scope.specialitybonus = rewardDatae.specialitybonus;
                $scope.totalcoins = rewardDatae.totalcoins;

                var image = $w.document.getElementById('rewardCharImg');
                ...
                downloadingImage.src = rewardDatae.imagecharachter;
                $ionicLoading.hide();
            });
    ...
    angular.module('gamesapp').controller('ShareCtrl', ['$scope', '$rootScope', '$http', '$ionicLoading', ShareCtrl]);
})(window);
...
(function SpreadingReward(win) {
    ...
    function renderRewardImage(callback) {
        ...
        loader.load(function (loader, resources) {
            ...
            var rarityText = new PIXI.Text('RARITY', {
                fontSize: '16px', fill: 0x2e2a0f, align: 'center', fontWeight: 'normal',
                letterSpacing: 3
            });
            ...
            var charcodeText = new PIXI.Text(rewardInfo.codechar, {
                fontSize: '20px', fill: 0xffffff, align: 'center', fontWeight: 'normal',
                letterSpacing: 2
            });
            ...
            var nicknameText = new PIXI.Text(rewardInfo.nickname, {
                fontSize: '20px', fill: 0x2e2a0f, align: 'center', fontWeight: 'normal',
                letterSpacing: 3
            });
            ...
            function Rowbartext(ctl, ctr, isShowCoin) {
                ...
                var text1 = new PIXI.Text(ctl, {
                    fontSize: '16px', fill: '#000000', align: 'center', fontWeight: 'normal'
                });
            ...
            var summaryText = new PIXI.Text("EGGCODE SUMMARY", {
                fontSize: '20px', fill: 0x000, align: 'center', fontWeight: 'normal',
            });
            ...
            var footerText1 = new PIXI.Text('FOR MORE INFO ABOUT YOUR EGGCODE DETAILS', {
                fontSize: '16px', fill: '#caa16d', align: 'center', fontWeight: 'normal'
            });
        ...
        if (window.plugins && window.plugins.socialsharing) {
            switch (socmedname) {
                ...
                case 'path':
                    ...
                    window.plugins.socialsharing.canShareVia('com.path', message(), subject(), fileOrFileArray(), url(), function(){
                        window.plugins.socialsharing.shareVia('com.path', message(), subject(), fileOrFileArray(), url(), successCallback, errorCallback);
                    }, errorCallback);
                    break;
                default:
    ...
    sr.savePhoto = function () {
        ...
        cordova.base64ToGallery(
            base64Data,
            {
                prefix: rewardInfo.codechar + '_hatchmarket_reward_',
                mediaScanner: true
            },
...
(function SpreadingEggcode(win) {
    ...
    function successCallback() {
        window.navigator.notification.confirm('You finished tutorial.', function (confirm) {
            if (confirm == 1) {
                window.location = 'main.html';
            }
        }, 'Welcome new Hatcher !', ['Exit', 'Later']);
    };
    function errorCallback(e) {
        console.log(e);
        window.GameApp.SpreadCode.shareToSocialMedia('default');
